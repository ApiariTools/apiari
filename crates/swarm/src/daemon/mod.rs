...
struct WorkspaceState {
    ...
    inbox_offset: u64,
    /// Whether to auto-close workers when their PR reaches a terminal state.
    ...
    for result in results {
            ...
            && let Some(worker) = ws.workers.get_mut(&result.worker_id)
        {
            ...
            *state_dirty = true;
            ...
            // Auto-close workers whose PR has reached a terminal state
            // (if enabled for this workspace).
            if should_auto_close && ws.close_on_pr_merge {
                tracing::info!(
                    worker_id = %worker.id,
                    pr_state = %worker.pr.as_ref().unwrap().state,
                    pr_number = worker.pr.as_ref().unwrap().number,
                    "Auto-closing worker, PR no longer open",
                );
                worker.message_tx = None;
                worker.phase = WorkerPhase::Completed;
                let _ = git::remove_worktree(&worker.repo_path, &worker.worktree_path);
...
mod tests {
    ...
    #[test]
    fn apply_pr_poll_results_skips_closed_pr_when_disabled() {
        let mut workspaces = HashMap::new();
        let ws_path = PathBuf::from("/tmp/ws");
        let mut ws = test_workspace("/tmp/ws", vec!["w-1"]);
        ws.close_on_pr_merge = false;
