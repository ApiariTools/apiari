...
type ResultItem = { kind: "worker"; worker: WorkerV2 } | { kind: "auto_bot"; bot: AutoBot };
...
export default function CommandPalette({
  ...
  const filteredBots = autoBots.filter((b) => matches(query, b.name));
              ...
              {filteredWorkers.map((w, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    {w.branch && <span className={styles.resultSub}>{w.branch}</span>}
                  </button>
                );
              })}
            </div>
