...
import type { WorkerV2, AutoBot, DashboardWidget, Repo, ContextBotContext } from "@apiari/types";
import { listWidgets, getRepos } from "@apiari/api";
        ...
        {[
          ...
          .map((s) => (
            <div key={s.label} className={styles.statPill}>
              <span className={styles.statPillNum} style={{ color: s.color }}>
                {s.count}
              </span>
      ...
      {attentionWorkers.length > 0 && (
        <div className={styles.attentionList}>
          ...
          {attentionWorkers.map((w) => {
            const dotColor =
              w.state === "stalled" ? "var(--status-stalled)" : "var(--status-waiting)";
            return (
              <button
                key={w.id}
          ...
          {recentWorkers.slice(0, 8).map((w) => (
            <button key={w.id} className={styles.attentionRow} onClick={() => onSelectWorker(w.id)}>
              <span
                ...
                style={{
                  background:
                    w.state === "abandoned" ? "var(--status-abandoned)" : "var(--status-merged)",
                }}
              />
              <span className={styles.attentionName}>{getWorkerTitle(w)}</span>
              <span
...
export default function Dashboard({
  ...
  const fetchWidgets = useCallback(() => {
    if (!workspace) return;
    listWidgets(workspace)
      .then(setWidgets)
      .catch((e) => console.error("[dashboard] fetch widgets:", e));
  }, [workspace]);
      ...
      {widgets.length === 0 && (
        <div className={styles.widgetsEmpty}>
          ...
          <p className={styles.widgetsEmptyHint}>
            Bots can write widgets to this dashboard using the <code>write_dashboard_widget</code>{" "}
            tool.
          </p>
        </div>
      )}
